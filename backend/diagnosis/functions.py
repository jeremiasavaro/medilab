...
def create_diagnosis_pdf(patient_name, diagnosis_date, diseases_accepted):
    # Path to the JSON file with disease descriptions
    base_dir = os.path.dirname(os.path.abspath(__file__))
    json_path = os.path.join(base_dir, '../diagnosis_data.json')

    # Load descriptions from JSON file
    with open(json_path, 'r', encoding='utf-8') as json_file:
        disease_descriptions = json.load(json_file)
    ...
    if diseases_accepted:
        ...
        # Add additional descriptions from the JSON
        elements.append(Paragraph("Additional Information:", header_style))
        for disease, _ in diseases_accepted:
            disease_key = disease.lower()
            if disease_key in disease_descriptions:
                description = disease_descriptions[disease_key].get("description", "No description available.")
                treatment = disease_descriptions[disease_key].get("treatment", "No treatment information available.")
                elements.append(Paragraph(f"<b>{disease_descriptions[disease_key]['title']}:</b>", body_style))
                elements.append(Paragraph(description, body_style))
                elements.append(Paragraph(f"<b>Treatment:</b> {treatment}", body_style))
        ...
        elements.append(Spacer(1, 10))
        elements.append(Paragraph(disease_descriptions["healthy"]["description"], body_style))
        elements.append(Paragraph(f"<b>Treatment:</b> {disease_descriptions['healthy']['treatment']}", body_style))
